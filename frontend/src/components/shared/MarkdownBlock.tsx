    ...
    <span key={key}>
      {parts.map((p, i) =>
        p.startsWith("**") && p.endsWith("**") ? (
          <strong key={i} className="font-semibold text-aegis-text">
            {p.slice(2, -2)}
          </strong>
        ) : (
          <span key={i}>{p}</span>
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("# ")) {
      nodes.push(
        <h2
          key={i}
          className="mt-4 mb-2 font-display text-sm font-bold text-aegis-text"
        >
    ...
    if (line.startsWith("## ")) {
      nodes.push(
        <h3
          key={i}
          className="mt-4 mb-1.5 font-display text-xs font-bold uppercase tracking-wide text-aegis-accent"
        >
    ...
    if (line.startsWith("### ")) {
      nodes.push(
        <h4
          key={i}
          className="mt-3 mb-1 font-display text-xs font-semibold text-aegis-text"
        >
    ...
    if (line.startsWith("- ") || line.startsWith("* ")) {
      ...
      while (
        ...
        items.push(
          <li
            key={i}
            className="flex gap-2 text-xs leading-relaxed text-aegis-muted"
          >
    ...
    if (line.startsWith("**") && line.endsWith("**")) {
      nodes.push(
        <p
          key={i}
          className="my-2 text-xs font-semibold leading-relaxed text-aegis-text"
        >
          {line.slice(2, -2)}
        </p>,
      );
