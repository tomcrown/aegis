...
function StatCard({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div className={`card p-5 ${accent ? "border-aegis-accent/30" : ""}`}>
      ...
      <div className={`font-display text-2xl font-bold ${accent ? "text-aegis-accent" : "text-aegis-text"}`}>
        {value}
      </div>
      {sub && <div className="mt-0.5 font-mono text-xs text-aegis-muted">{sub}</div>}
    </div>
  );
}
    ...
    <div className="flex flex-col items-center gap-3 py-12">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-aegis-border bg-aegis-surface2">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
          <path d="M10 2L3 5.5V10C3 14 6.2 17.6 10 18.5C13.8 17.6 17 14 17 10V5.5L10 2Z"
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard label="Total Value Locked" value={fmt(vaultState?.total_tvl)} accent />
          <StatCard label="Active Protections" value={String(vaultState?.active_protections ?? 0)} />
          <StatCard label="Users Protected" value={String(vaultState?.user_count ?? 0)} />
          <StatCard label="Yield Distributed" value={fmt(vaultState?.total_yield_distributed)} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="card p-5">
            ...
            <div className="mt-0.5 font-mono text-xs text-aegis-muted">0.1% of notional</div>
          </div>
          <div className="card p-5">
            ...
            <div className="mt-0.5 font-mono text-xs text-aegis-muted">Funding rate yield</div>
          </div>
          <div className="card p-5">
            ...
            <div className="mt-0.5 font-mono text-xs text-aegis-muted">Open hedge orders</div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 rounded-full border border-aegis-accent/20 bg-aegis-accent/5 px-3 py-1">
            ...
            <span className="font-mono text-[10px] text-aegis-accent">builder_code=AEGIS</span>
          </div>
        </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-aegis-border">
                    {["Symbol", "Action", "Amount", "Price", "Notional", "PnL"].map((h) => (
                ...
                <tbody className="divide-y divide-aegis-border/50">
                  {(trades as Record<string, string>[]).map((t, i) => {
                    ...
                    const notional = parseFloat(t["amount"] ?? "0") * parseFloat(t["price"] ?? "0");
                    return (
                      <tr key={i} className="group hover:bg-aegis-surface2 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-display font-semibold text-aegis-text">{t["symbol"]}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`rounded px-2 py-0.5 font-display text-[10px] font-semibold ${
                            t["side"] === "ask"
                              ? "bg-aegis-red/10 text-aegis-red"
                              : "bg-aegis-green/10 text-aegis-green"
                          }`}>
                            {t["side"] === "ask" ? "↓ SHORT" : "↑ LONG"}
                          </span>
                        </td>
                        <td className="px-4 py-3 font-mono text-aegis-text">{t["amount"]}</td>
                        <td className="px-4 py-3 font-mono text-aegis-text">${t["price"]}</td>
                        <td className="px-4 py-3 font-mono text-aegis-muted">
                          ${notional.toLocaleString("en-US", { maximumFractionDigits: 0 })}
                        </td>
                        <td className={`px-4 py-3 font-mono font-semibold ${pnl >= 0 ? "text-aegis-green" : "text-aegis-red"}`}>
                          {pnl >= 0 ? "+" : ""}{pnl.toFixed(2)}
                        </td>
                      </tr>
                    );
      ...
      <div className="card p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            { step: "01", title: "Premium Collection", desc: "When you activate Aegis, a 0.1% protection premium on your notional position value enters the vault." },
            { step: "02", title: "Yield Generation", desc: "When Aegis places hedge orders, the position earns funding rate yield. This accrues to vault participants." },
            { step: "03", title: "Verifiable On-Chain", desc: "Every hedge trade is attributed to builder_code=AEGIS on Pacifica. Full audit trail visible in the table above." },
          ].map(({ step, title, desc }) => (
            <div key={step} className="space-y-2">
              ...
              <h3 className="font-display text-sm font-semibold text-aegis-text">{title}</h3>
