...
function StatCard({
  ...
  return (
    <div className={`card p-5 ${accent ? "border-aegis-accent/30 bg-aegis-accent/[0.03]" : ""}`}>
      <div className="label mb-1">{label}</div>
      <div className={`font-display text-2xl font-bold ${color ?? (accent ? "text-aegis-accent" : "text-aegis-text")}`}>
        {value}
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="card p-5">
          ...
          <div className="mt-1 font-mono text-xs text-aegis-muted">0.1% of your position notional</div>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-aegis-surface2">
            <div className="h-full rounded-full bg-aegis-accent" style={{ width: "100%" }} />
          </div>
        </div>
        ...
        <div className="card p-5">
          ...
          <div className="mt-1 font-mono text-xs text-aegis-muted">Funding rate on hedge positions</div>
          <div className="mt-3 flex items-center gap-1.5">
            <span className="dot-green" />
            <span className="font-mono text-[10px] text-aegis-muted">Accruing in real-time</span>
          </div>
        </div>
        ...
        <div className="card p-5">
          ...
          <div className="mt-1 font-mono text-xs text-aegis-muted">Open protective orders</div>
          <div className="mt-3">
      ...
      <div className="card overflow-hidden">
        {trades.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-sm font-medium text-aegis-muted">No trades yet</p>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-aegis-border bg-aegis-surface2">
                  {["Token", "Direction", "Size", "Avg Price", "Value", "PnL"].map((h) => (
                    ...
                    <tr key={i} className="group hover:bg-aegis-surface2 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`rounded px-2 py-0.5 font-display text-[10px] font-semibold ${
                          isShort ? "bg-aegis-red/10 text-aegis-red" : "bg-aegis-green/10 text-aegis-green"
                        }`}>
                          {isShort ? "↓ Short" : "↑ Long"}
                        </span>
                      </td>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {steps.map(({ icon, title, desc }) => (
          <div key={title} className="flex gap-3">
            ...
            <div>
              <h4 className="font-display text-sm font-semibold text-aegis-text">{title}</h4>
              <p className="mt-1 text-xs leading-relaxed text-aegis-muted">{desc}</p>
            </div>
          </div>
        ))}
      </div>
...
function ApiSetupCard() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const header = { expiry_window: expiryWindow, timestamp, type: "create_api_key" };
      ...
      setStatus({ ok: true, msg: `Key saved: ${result.api_key.slice(0, 16)}...` });
    } catch (err) {
  ...
  return (
    <div className="card p-6">
      <div className="mb-4">
        <h3 className="font-display text-base font-bold text-aegis-text">Rate Limit Key</h3>
        <p className="mt-1 text-sm text-aegis-muted">
          Generate a Pacifica API Config Key to increase rate limits for the Aegis backend. One-time setup — sign with your wallet.
        </p>
      </div>
      <button
      ...
      {status && (
        <div className={`mt-3 rounded-lg border px-3 py-2 text-center font-mono text-xs ${
          status.ok
